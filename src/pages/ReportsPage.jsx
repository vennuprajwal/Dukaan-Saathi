    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {report.summary.map((entry) => (
          <div key={entry.label} className="rounded-2xl bg-white p-4 shadow-[var(--shadow-card)] ring-1 ring-black/5">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink/50">{entry.label}</p>
            <p className="mt-1 font-display text-xl font-semibold text-shopfront">{entry.value}</p>
          </div>
        ))}
      </div>
