  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        owner_name: ownerName.trim() || "Owner",
        mobile_number: mobileNumber.trim(),
        email: email.trim(),
        shop_address: shopAddress.trim(),
        shop_logo: shopLogo.trim(),
        business_category: businessCategory.trim() || "General Store",
    ...
    <div className="flex min-h-screen items-center justify-center bg-paper px-5 py-10 font-body text-ink">
      <div className="w-full max-w-md">
        <div className="mb-8 flex items-center justify-between">
          <Link
            to="/"
            className="inline-flex items-center gap-2 text-sm text-ink/60 hover:text-shopfront transition-colors"
          >
        ...
        >
            ...
            <>
              <div className="mb-8 flex flex-col items-center text-center">
                ...
                <p className="mt-2 text-sm text-ink/60">
                  {t(
                    ...
                    "Register your shop to start using Dukaan Saathi."
                  )}
                </p>
              </div>
              <form onSubmit={handleRegister} className="space-y-3">
                ...
                <input
                  ...
                  placeholder={t("auth.owner_name", "Owner Name")}
                  disabled={busy}
                  className="w-full rounded-full border border-black/10 bg-paper px-5 py-3.5 text-sm text-ink outline-none transition-all focus:border-leaf focus:bg-white focus:ring-4 focus:ring-leaf/10 disabled:opacity-50"
                />
                <input
                  ...
                  placeholder={t("auth.mobile_number", "Mobile Number (WhatsApp)")}
                  disabled={busy}
                  className="w-full rounded-full border border-black/10 bg-paper px-5 py-3.5 text-sm text-ink outline-none transition-all focus:border-leaf focus:bg-white focus:ring-4 focus:ring-leaf/10 disabled:opacity-50"
                />
                <input
                  ...
                  placeholder={t("auth.email_optional", "Email (Optional)")}
                  disabled={busy}
                  className="w-full rounded-full border border-black/10 bg-paper px-5 py-3.5 text-sm text-ink outline-none transition-all focus:border-leaf focus:bg-white focus:ring-4 focus:ring-leaf/10 disabled:opacity-50"
                />
                <input
                ...
                <input
                  ...
                  placeholder={t("auth.shop_logo_optional", "Shop Logo URL (Optional)")}
                  disabled={busy}
                  className="w-full rounded-full border border-black/10 bg-paper px-5 py-3.5 text-sm text-ink outline-none transition-all focus:border-leaf focus:bg-white focus:ring-4 focus:ring-leaf/10 disabled:opacity-50"
                />
                {/* Account credentials */}
                ...
                >
                  {busy ? t("common.loading", "Loading…") : t("auth.register", "Register")}
                </button>
          ...
          {error && (
            <motion.p
              ...
              className="mt-4 rounded-xl bg-terracotta/10 px-4 py-3.5 text-sm text-terracotta"
            >
              {error}
            </motion.p>
          )}
